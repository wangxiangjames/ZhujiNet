...
@property(nonatomic, strong) NSMutableCharacterSet *urlEncodeSet;
...
- (instancetype)initWithConfig:(TVCConfig *)config {
    ...
    if (self) {
        self.config = config;
        self.urlEncodeSet = [[NSMutableCharacterSet alloc] init];
        [self.urlEncodeSet formUnionWithCharacterSet:[NSCharacterSet URLQueryAllowedCharacterSet]];
        [self.urlEncodeSet removeCharactersInString:@"/+=\n"];
        self.reqKey = @"";
        self.serverIP = @"";
    }
    return self;
    ...
    if ([manager fileExistsAtPath:param.videoPath]) {
        ...
        if (uploadContext.isUploadCover) {
            if ([manager fileExistsAtPath:param.coverPath]) {
                ...
                [self txReport:TVC_UPLOAD_EVENT_ID_INIT errCode:TVC_ERR_FILE_NOT_EXIST errInfo:@"coverPath is not exist" reqTime:reqTime reqTimeCost:0 reqKey:@"" appId:0 fileSize:0 fileType:[self getFileType:param.coverPath] fileName:[self getFileName:param.coverPath] sessionKey:@""];
        ...
        [self txReport:TVC_UPLOAD_EVENT_ID_INIT errCode:TVC_ERR_FILE_NOT_EXIST errInfo:@"videoPath is not exist" reqTime:reqTime reqTimeCost:0 reqKey:@"" appId:0 fileSize:0 fileType:[self getFileType:param.videoPath] fileName:[self getFileName:param.videoPath] sessionKey:@""];
...
- (NSMutableURLRequest *)getCosInitURLRequest:(TVCUploadContext *)uploadContext withVodSessionKey:(NSString *)vodSessionKey {
    ...
    if (vodSessionKey && vodSessionKey.length) {
        ...
        [dictParam setValue:[self getFileName:param.videoPath] forKey:@"videoName"];
    ...
    NSURLSessionTask *initTask = [initSess dataTaskWithRequest:cosRequest completionHandler:^(NSData *_Nullable initData, NSURLResponse *_Nullable response, NSError *_Nullable error) {
        ...
        if (jsonErr || ![initDict isKindOfClass:[NSDictionary class]]) {
            ...
            [ws txReport:TVC_UPLOAD_EVENT_ID_INIT errCode:rsp.retCode errInfo:rsp.descMsg reqTime:uploadContext.reqTime reqTimeCost:reqTimeCost reqKey:ws.reqKey appId:0 fileSize:uploadContext.videoSize
                fileType:[ws getFileType:uploadContext.uploadParam.videoPath] fileName:[ws getFileName:uploadContext.uploadParam.videoPath] sessionKey:@""];
        ...
        [ws txReport:TVC_UPLOAD_EVENT_ID_INIT errCode:TVC_OK errInfo:@"" reqTime:uploadContext.reqTime reqTimeCost:reqTimeCost reqKey:ws.reqKey appId:ugc.userAppid fileSize:uploadContext.videoSize
            fileType:[ws getFileType:uploadContext.uploadParam.videoPath] fileName:[ws getFileName:uploadContext.uploadParam.videoPath] sessionKey:ugc.uploadSession];
            ...
            [videoUpload setFinishBlock:^(QCloudUploadObjectResult *result, NSError * error) {
                ...
                if (error) {
                    ...
                    [ws txReport:TVC_UPLOAD_EVENT_ID_COS errCode:TVC_ERR_VIDEO_UPLOAD_FAILED errInfo:errInfo reqTime:uploadContext.reqTime reqTimeCost:reqTimeCost reqKey:ws.reqKey appId:cug.userAppid fileSize:uploadContext.videoSize
                        fileType:[ws getFileType:uploadContext.uploadParam.videoPath] fileName:[ws getFileName:uploadContext.uploadParam.videoPath] sessionKey:cug.uploadSession];
                    // 取消的情况不清除session缓存
                    if (error.code == -34009) {
                        ...
                        uploadContext.desc = [NSString stringWithFormat:@"upload video, cos code:%d, cos desc:%@", error.code, error.localizedDescription];
                        //网络断开，不清除session缓存
                        if (error.code != -34004) {
...
- (void) txReport:(int)eventId errCode:(int)errCode errInfo:(NSString*)errInfo reqTime:(int64_t)reqTime reqTimeCost:(int64_t)reqTimeCost reqKey:(NSString*)reqKey appId:(NSString*)appId fileSize:(int64_t)fileSize fileType:(NSString*)fileType fileName:(NSString*)fileName sessionKey:(NSString*)sessionKey
{
    // set url
    ...
    [dictParam setValue:[NSNumber numberWithInt:eventId] forKey:@"reqType"];
    [dictParam setValue:[NSNumber numberWithInt:errCode]  forKey:@"errCode"];
    [dictParam setValue:errInfo forKey:@"errMsg"];
    [dictParam setValue:[NSNumber numberWithLongLong:reqTimeCost] forKey:@"reqTimeCost"];
    [dictParam setValue:[NSNumber numberWithLongLong:reqTime]  forKey:@"reqTime"];
    [dictParam setValue:self.serverIP forKey:@"reqServerIp"];
    ...
    [dictParam setValue:self.config.userID  forKey:@"reportId"];
    [dictParam setValue:[TVCUtils tvc_getDevUUID] forKey:@"uuid"];
    [dictParam setValue:reqKey forKey:@"reqKey"];
    [dictParam setValue:[NSNumber numberWithLongLong:[appId longLongValue]] forKey:@"appId"];
    [dictParam setValue:[NSNumber numberWithLongLong:fileSize] forKey:@"fileSize"];
    [dictParam setValue:fileType forKey:@"fileType"];
    [dictParam setValue:fileName forKey:@"fileName"];
    [dictParam setValue:sessionKey forKey:@"vodSessionKey"];
    ...
    NSURLSession *initSess = [NSURLSession sessionWithConfiguration:initCfg delegate:[[TVCHttpsDelegate alloc] init] delegateQueue:nil];
    NSURLSessionTask *initTask = [initSess dataTaskWithRequest:request completionHandler:nil];
    [initTask resume];
