...
-(void)setItemName:(NSString *)itemName{
    ...
    [self setTitleColor:RGBColor(22.0, 22.0, 22.0) forState:0];
   ...
   forControlEvents:1<<6];
    ...
    if (![itemName isEqualToString:kGuDing]) {
        if (!self.deleteBtn) {
            self.deleteBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width-kDeleteW, -kDeleteW+2, 2*kDeleteW, 2*kDeleteW)];
...
-(void)sortButtonClick{
    ...
    if (self.hiddenBtn.hidden && self.location == top && ![self.itemName isEqualToString:kGuDing]) {
        [self addGestureRecognizer:self.gesture];
    }
...
-(void)operationWithoutHidBtn{
    if (self.location == top && ![self.itemName isEqualToString:kGuDing]){
...
- (void)animationForWholeView{
    for (int i = 0; i <topView.count; i++) {
        [self animationWithView:[topView objectAtIndex:i] frame:CGRectMake(padding+(padding+kItemW)*(i%itemPerLine), padding+(padding+kItemH)*(i/itemPerLine)+10, kItemW, kItemH)];
    }
    for (int i = 0; i < bottomView.count; i++) {
        [self animationWithView:[bottomView objectAtIndex:i] frame:CGRectMake(padding+(padding+kItemW)*(i%itemPerLine), [self TopViewMaxY]+60+(kItemH+padding)*(i/itemPerLine), kItemW, kItemH)];
    }
    [self animationWithView:self.hitTextLabel frame:CGRectMake(0,[self TopViewMaxY],kScreenW,30)];
}
