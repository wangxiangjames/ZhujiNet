...
{
    if (delegate != NULL) {
        [delegate cjwAlertViewButtonTouchUpInside:self clickedButtonAtIndex:[sender tag]];
    }
...
- (void)cjwAlertViewButtonTouchUpInside: (CjwAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        [closeButton addTarget:self action:@selector(cjwAlertViewButtonTouchUpInside:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if ([touch.view isKindOfClass:[CjwAlertView class]]) {
        [self close];
    }
}
