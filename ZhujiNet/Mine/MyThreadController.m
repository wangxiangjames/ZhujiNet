...
//  ZhujiNet
//
//  Created by zhujiribao on 2018/3/10.
...
@interface MyThreadController()<UITableViewDelegate,UITableViewDataSource>{
    AppDelegate                 *_app;
    CommonCell                  *_cell;
    CjwItem                     *_cjwItem;
...
- (void)viewDidLoad {
    ...
    _url=URL_mythread;
    if(self.isMythread==YES){
        _threadTypeParam=@"";
        self.navigationItem.title = @"我的帖子";
    }
    else{
        _threadTypeParam=@"reply";
...
-(void)addTableView{
    ...
    NSDictionary *param=@{@"type":_threadTypeParam,@"page":[NSNumber numberWithLong:_page]};
    [_app.net request:self url:_url param:param];
}
    ...
    if ([status[@"stat"] isEqual:@0]) {
        ...
        if(dict[@"data"]!=nil){
            ...
            for(id item in digest){
                ...
                cjwItem.from=self.isMythread?item[@"forumname"]:@"";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    detail.contmode=1;
    detail.artTitle=self.isMythread==NO ? @"":item.title;
